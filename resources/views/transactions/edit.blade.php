...
<div class="container mx-auto px-4 py-6 max-w-3xl">
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600">Ubah detail transaksi</p>
    </div>
        ...
        <form action="{{ route('transactions.update', $transaction->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="barcode" name="barcode" value="{{ $transaction->barcode }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required readonly>
                </div>
                ...
                <div>
                    ...
                    <select id="transaction_type" name="transaction_type" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                        <option value="IN" {{ $transaction->transaction_type == 'IN' ? 'selected' : '' }}>Masuk</option>
                        <option value="OUT" {{ $transaction->transaction_type == 'OUT' ? 'selected' : '' }}>Keluar</option>
                    </select>
                </div>

                <div>
                    <label for="quantity" class="block text-sm font-medium text-gray-700 mb-1">Jumlah</label>
                    <input type="number" id="quantity" name="quantity" min="1" value="{{ $transaction->quantity }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                </div>

                <div>
                    <label for="user_name" class="block text-sm font-medium text-gray-700 mb-1">Nama User</label>
                    <input type="text" id="user_name" name="user_name" value="{{ $transaction->user_name }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>
                </div>

                <div>
                    <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Catatan</label>
                    <textarea id="notes" name="notes" rows="3" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ $transaction->notes }}</textarea>
                </div>
            </div>
